...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "modif_modele_pc")) {
  $updateSQL = sprintf("UPDATE dmic_modele_pc SET name_modele=%s, description_modele=%s WHERE ID=%s",
  ...
  $updateGoTo = "http://www.dmic-corp.fr/front/admin/config_intitules_modele_pc.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING'];
  }
  header(sprintf("Location: http://www.dmic-corp.fr/front/admin/config_intitules_modele_pc.php"));
}

mysql_select_db($database_dmic, $dmic);
$query_modele_ordinateur = "SELECT * FROM dmic_modele_pc ORDER BY name_modele ASC";
$modele_ordinateur = mysql_query($query_modele_ordinateur, $dmic) or die(mysql_error());
$row_modele_ordinateur = mysql_fetch_assoc($modele_ordinateur);
$totalRows_modele_ordinateur = mysql_num_rows($modele_ordinateur);

$colname_modele_ordinateur_dyn = "-1";
if (isset($_GET['id'])) {
  $colname_modele_ordinateur_dyn = $_GET['id'];
}
mysql_select_db($database_dmic, $dmic);
$query_modele_ordinateur_dyn = sprintf("SELECT * FROM dmic_modele_pc WHERE ID = %s", GetSQLValueString($colname_modele_ordinateur_dyn, "int"));
$modele_ordinateur_dyn = mysql_query($query_modele_ordinateur_dyn, $dmic) or die(mysql_error());
$row_modele_ordinateur_dyn = mysql_fetch_assoc($modele_ordinateur_dyn);
$totalRows_modele_ordinateur_dyn = mysql_num_rows($modele_ordinateur_dyn);
...
<div id="LayoutDiv4" class="content"><!-- InstanceBeginEditable name="content" -->
    ...
    <h2 align="center" class="Style7">Modification d'un Mod&egrave;le Ordinateur.</h2>
    <form action="<?php echo $editFormAction; ?>" method="POST" name="modif_modele_pc" id="modif_modele_pc">
      <p>&nbsp;</p>
    <table width="847" border="1" align="center" bordercolor="#92BEE1" class="tab_cadrehov">
      <tr>
        ...
        <th width="600" bordercolor="#92BEE1" scope="col"><div align="center" class="Style4"><span class="Style7"><?php echo $row_modele_ordinateur_dyn['ID']; ?></span>
          <input name="id" type="hidden" id="id" value="<?php echo $row_modele_ordinateur_dyn['ID']; ?>" />
          </div></th>
        </tr>
      <tr>
        <th bordercolor="#92BEE1" scope="row"><div align="right" class="Style7">Nom du profil</div></th>
        <td bordercolor="#92BEE1"><label>
          <input name="new_name" type="text" id="new_name" value="<?php echo $row_modele_ordinateur_dyn['name_modele']; ?>" size="100" maxlength="250" />
          </label></td>
        </tr>
      <tr>
...
<div class='center spaced'><textarea name='new_commentaire' cols='100' rows='30' id='new_commentaire' onBlur="MM_validateForm('answer','','R');return document.MM_returnValue"><?php echo $row_modele_ordinateur_dyn['description_modele']; ?></textarea>
</div>
          </label></td>
        </tr>
      <tr>
    ...
    <input type="hidden" name="MM_update" value="modif_modele_pc" />
  </form>
  <h1 class="Style5"></h1>
  <h1>&nbsp;</h1>
  <!-- end .content -->
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($modele_ordinateur);

mysql_free_result($modele_ordinateur_dyn);
?>
