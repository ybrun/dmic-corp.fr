...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "modif_os")) {
  $updateSQL = sprintf("UPDATE dmic_os SET name=%s, `description`=%s WHERE id_os=%s",
  ...
  $updateGoTo = "http://www.dmic-corp.fr/front/admin/config_intitules_os.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING'];
  }
  header(sprintf("Location: http://www.dmic-corp.fr/front/admin/config_intitules_os.php"));
}
...
$colname_os_dyn = "-1";
if (isset($_GET['id'])) {
  $colname_os_dyn = $_GET['id'];
}
mysql_select_db($database_dmic, $dmic);
$query_os_dyn = sprintf("SELECT * FROM dmic_os WHERE id_os = %s", GetSQLValueString($colname_os_dyn, "int"));
$os_dyn = mysql_query($query_os_dyn, $dmic) or die(mysql_error());
$row_os_dyn = mysql_fetch_assoc($os_dyn);
$totalRows_os_dyn = mysql_num_rows($os_dyn);
...
<div id="LayoutDiv4" class="content"><!-- InstanceBeginEditable name="content" -->
    ...
    <h2 align="center" class="Style7">Modification d'un Syst&egrave;me d'exploitation.</h2>
    <form action="<?php echo $editFormAction; ?>" method="POST" name="modif_os" id="modif_os">
      <p>&nbsp;</p>
    <table width="847" border="1" align="center" bordercolor="#92BEE1" class="tab_cadrehov">
      <tr>
        ...
        <th width="600" bordercolor="#92BEE1" scope="col"><div align="center" class="Style4"><span class="Style7"><?php echo $row_modele_pc_dyn['ID']; ?></span>
          <input name="id" type="hidden" id="id" value="<?php echo $row_os_dyn['id_os']; ?>" />
          </div></th>
        </tr>
      <tr>
        <th bordercolor="#92BEE1" scope="row"><div align="right" class="Style7">Nom de l'OS</div></th>
        <td bordercolor="#92BEE1"><label>
          <input name="new_name" type="text" id="new_name" value="<?php echo $row_os_dyn['name']; ?>" size="100" maxlength="250" />
          </label></td>
        </tr>
      <tr>
...
<div class='center spaced'><textarea name='new_commentaire' cols='100' rows='30' id='new_commentaire' onBlur="MM_validateForm('answer','','R');return document.MM_returnValue"><?php echo $row_os_dyn['description']; ?></textarea>
</div>
          </label></td>
        </tr>
      <tr>
    ...
    <input type="hidden" name="MM_update" value="modif_os">
  </form>
  <h1 class="Style5"></h1>
  <h1>&nbsp;</h1>
  <h1>&nbsp;</h1>
  <!-- end .content -->
<?php echo $row_os_dyn['id_os']; ?>
<!-- InstanceEndEditable --></div>
