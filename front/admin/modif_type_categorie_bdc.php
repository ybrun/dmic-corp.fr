...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "modif_type_categorie")) {
  $updateSQL = sprintf("UPDATE dmic_knowbaseitemtypecategorie SET name_type=%s, commentaire_type=%s WHERE id=%s",
  ...
  $updateGoTo = "http://www.dmic-corp.fr/front/admin/config_intitules_type_categorie_bdc.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING'];
  }
  header(sprintf("Location: http://www.dmic-corp.fr/front/admin/config_intitules_type_categorie_bdc.php"));
}

$colname_type_categorie_dyn = "-1";
if (isset($_GET['id'])) {
  $colname_type_categorie_dyn = $_GET['id'];
}
mysql_select_db($database_dmic, $dmic);
$query_type_categorie_dyn = sprintf("SELECT * FROM dmic_knowbaseitemtypecategorie WHERE id = %s", GetSQLValueString($colname_type_categorie_dyn, "int"));
$type_categorie_dyn = mysql_query($query_type_categorie_dyn, $dmic) or die(mysql_error());
$row_type_categorie_dyn = mysql_fetch_assoc($type_categorie_dyn);
$totalRows_type_categorie_dyn = mysql_num_rows($type_categorie_dyn);
...
<div id="LayoutDiv4" class="content"><!-- InstanceBeginEditable name="content" -->
    ...
    <h2 align="center" class="Style7">Modification d'un type de cat&eacute;gorie de base de connaissances.</h2>
    <form action="<?php echo $editFormAction; ?>" method="POST" name="modif_type_categorie" id="modif_type_categorie">
      <p>&nbsp;</p>
      <table width="847" border="1" align="center" bordercolor="#92BEE1" class="tab_cadrehov">
        <tr>
          ...
          <th width="600" bordercolor="#92BEE1" scope="col"><div align="center" class="Style4"><span class="Style7"><?php echo $row_type_categorie_dyn['id']; ?></span>
            <input name="id" type="hidden" id="id" value="<?php echo $row_type_categorie_dyn['id']; ?>" />
            </div></th>
          </tr>
        <tr>
          ...
          <td bordercolor="#92BEE1"><label>
            <input name="new_name" type="text" id="new_name" value="<?php echo $row_type_categorie_dyn['name_type']; ?>" size="100" maxlength="250" />
            </label></td>
          </tr>
        <tr>
          <th bordercolor="#92BEE1" scope="row"><div align="right" class="Style7">
...
<div class='center spaced'><textarea name='new_commentaire' cols='100' rows='30' id='new_commentaire' onBlur="MM_validateForm('answer','','R');return document.MM_returnValue"><?php echo $row_type_categorie_dyn['commentaire_type']; ?></textarea>
</div>
            </label></td>
          </tr>
        <tr>
      ...
      <input type="hidden" name="MM_update" value="modif_type_categorie" />
    </form>
    <h1 class="Style5"></h1>
  <!-- end .content -->
<!-- InstanceEndEditable --></div>
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($type_categorie_dyn);
?>
