...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form")) {
  $insertSQL = sprintf("INSERT INTO dmic_proc_tech (proc_name, proc_finalname, proc_categorie, proc_description, proc_auteur) VALUES (%s, %s, %s, %s, %s)",
  ...
  $insertGoTo = "http://www.dmic-corp.fr/front/contribute/upload_proc_tech.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<div id="LayoutDiv4" class="content"><!-- InstanceBeginEditable name="content" -->
  <h1 align="center">Upload de proc&eacute;dures techniques.</h1>
  <p>&nbsp;</p>
  <form action="<?php echo $editFormAction; ?>" method="post" enctype="multipart/form-data">
    <div align="center">
      <table width="600" border="1" class="tab_cadrehov">
        <tr>
          <th width="300" scope="row">Fichier &agrave; uploader (pdf uniquement)</th>
          <td width="284"><input name="upload" type="file" /></td>
        </tr>
        <tr>
          <th scope="row">Nom de la proc&eacute;dure</th>
          <td><input type="text" name="procs_finalname" id="procs_finalname"></td>
        </tr>
        <tr>
          <th scope="row">Cat&eacute;gorie de la proc&eacute;dure</th>
          <td><select name="proc_categorie" id="proc_categorie">
            <?php
do {  
?>
            <option value="<?php echo $row_proc_categorie['name_categorie']?>"><?php echo $row_proc_categorie['name_categorie']?></option>
            <?php
...
<div class='center spaced'>
  <textarea name='procs_description' cols='25' rows='10' id='procs_description' onBlur="MM_validateForm('proc_description','','R');return document.MM_returnValue"></textarea></td>
        </tr>
        <tr>
