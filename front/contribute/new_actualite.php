...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "actualite")) {
  $insertSQL = sprintf("INSERT INTO dmic_actualites (`date`, titre, type, actualites) VALUES (%s, %s, %s, %s)",
                       GetSQLValueString($_POST['date_actualite'], "date"),
                       GetSQLValueString($_POST['titre'], "text"),
                       GetSQLValueString($_POST['section_site'], "text"),
                       GetSQLValueString($_POST['new_actualite'], "text"));

  mysql_select_db($database_dmic, $dmic);
  $Result1 = mysql_query($insertSQL, $dmic) or die(mysql_error());

  $insertGoTo = "http://www.dmic-corp.fr/front/index.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<div id="LayoutDiv4" class="content"><!-- InstanceBeginEditable name="content" -->
    <h1 align="center" class="Style7">Ajouter une actualit&eacute;.</h1>
            ...
            <table width="1000" border="1" class="tab_cadrehov">
				<tr class="noHover">
					<th width="50">
						<div align="center">
							...
							<input name="date_actualite" type="hidden" id="date_actualite" value="<?php echo $date; ?>" />
						</div>
					</th>
					<th width="441">
						...
						<em>( 
							...
							<?php
								do {  
							?>
							<option value="<?php echo $row_section['type_actualite']?>"><?php echo $row_section['type_actualite']?></option>
							<?php
							...
							<a href="http://www.dmic-corp.fr/front/admin/config_intitules_types_actualites.php" target="pop-up1" onClick="window.open('','pop-up1','width=1075,height=800,top=150,left=200,toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=0,resizable=0')"><img src="http://www.dmic-corp.fr/images/add_dropdown.png" width="11" height="11"></a>
						</em>
						<em>
						) </em></p>
					</th>
				...
				<tr>
					...
					<div class='center spaced'>
						<textarea name='new_actualite' cols='125' rows='15' id='new_actualite' onBlur="MM_validateForm('answer','','R');return document.MM_returnValue"><?php echo $row_news_dyn['actualites']; ?></textarea>
					</div>
					</label></div></td>
				</tr>
				<tr>
				  <td colspan="2"><input type="submit" name="News" id="News" value="Envoyer"></td>
			  </tr>
			</table>
		</div>
		<input type="hidden" name="MM_insert" value="actualite">
    </form>
    <p align="center" class="Style5">&nbsp;</p>
  <!-- end .content -->
<!-- InstanceEndEditable --></div>
